...
build_repo_command() {
  ...
  if [[ -f "${REPO_DIR}/flake.nix" ]]; then
    printf "cd '%s' && exec sudo NIX_CONFIG='experimental-features = nix-command flakes' /nix/var/nix/profiles/default/bin/nix develop --accept-flake-config --no-pure-eval -c bash -lc %q" \
      "${REPO_DIR}" "${inner}"
  else
